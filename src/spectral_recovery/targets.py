...
def _buffered_window_time_clip(
    timeseries, site, reference_start, reference_end, buffer
):
    ...
    if not all([p == 0 for p in req_padding]):
        raise BufferError(
            f"Buffer exceeds boundaries, padding required to use rolling window of size {(buffer*2)+1}",
            *req_padding,
        )

    buffered_clip = timeseries[
        :, :, y_back_step : y_front_step + 1, x_back_step : x_front_step + 1
    ]
...
def window_target(
    ...
    if na_rm:
        ...
        min_periods = None
    windowed_mean = median_time.rolling(
        {"y": N, "x": N}, center=True, min_periods=min_periods
    ).mean()
