...
class TestComputeIndices:
    def test_correct_call_from_index_input_to_function(self):
        ...
        with patch.dict(_indices_map, {Index.NBR: mock_nbr, Index.NDVI: mock_ndvi}):
            ...
            mock_nbr.assert_called_with(SAME_XR(input_xr))
            mock_ndvi.assert_called_with(SAME_XR(input_xr))
    ...
    def test_output_has_correct_band_dimension(self):
        ...
        mock_nbr = Mock(
            return_value=xr.DataArray(
                [[0]], dims=["time", "band"], coords={"band": ["band_1"]}
            )
        )
        mock_ndvi = Mock(
            return_value=xr.DataArray(
                [[0]], dims=["time", "band"], coords={"band": ["band_2"]}
            )
        )
        # Patch the map of index names to functions, replace the functions with mocks
        with patch.dict(_indices_map, {Index.NBR: mock_nbr, Index.NDVI: mock_ndvi}):
            input_xr = xr.DataArray([[[[0]]], [[[0]]]], dims=["band", "time", "y", "x"])
            res = compute_indices(
                input_xr,
                ["NBR", "NDVI"],
            )
